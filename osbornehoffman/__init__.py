...
class OsborneHoffmanConnection:
    ...
    async def __call__(
        ...
        while not reader.at_eof():
            ...
            _LOGGER.debug("Acknowledge: %s, Encrypted: %s", ack, event.get("encrypted_ack"))
...
class OsborneHoffmanTcpServer:
    ...
    def __init__(
        ...
        host: str,
        port: int,
        accounts: dict[str, OsborneHoffmanAccount],
    ...
    async def start_server(self):
        ...
        self.server = await asyncio.start_server(
            OsborneHoffmanConnection(self), host=self.host, port=self.port
        )
